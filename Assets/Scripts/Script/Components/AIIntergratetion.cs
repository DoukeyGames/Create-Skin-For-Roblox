...
public class AIIntergratetion : MonoBehaviour
{
    ...
    public void Download()
    {
        ...
        NativeGallery.Permission permission = NativeGallery.SaveImageToGallery(
            ...
            (success, path) => {
                ...
                if (success)
                {
                    MenuPanelController.Instance.notification.SetNotificationText("Image Saved", "Your generated image has been saved to the gallery.");
                }
                else
                {
                    MenuPanelController.Instance.notification.SetNotificationText("Save Failed", "Failed to save the image. Please try again.");
                }
    ...
    IEnumerator RequestGen(string prompt)
    {
        ...
        else
        {
            ...
            LoadImageFromResponse(www.downloadHandler.text);
            Debug.Log("Response: " + www.downloadHandler.text);
        }
    }
    
    public void LoadImageFromResponse(string json)
    {
        isLoading = false;
        loadingPanel.SetActive(false);
