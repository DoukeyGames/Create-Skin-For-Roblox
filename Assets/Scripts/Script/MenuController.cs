...
{
    public WallpaperHandler wallpaperHandler;
    public TextMeshProUGUI m_selectedText;
    public int m_maxShirts;
    public int m_maxPants;
    public int m_maxTShirts;
    public int m_maxSkins;
    public GridLayoutGroup m_SkinBookGridLayout;

    public GameObject m_cellPrefab,m_skinPrefab,m_fullbodycellPrefab, wallpaperPrefab;


    public Transform m_ShirtParent, m_PantParent,m_SkinParent,m_FullbodyParent, wallpaperParent;
    ...
    public BodyController m_bodyController;
    ...
    {
        if (WallpaperItemControllers.Count >= wallpaperHandler.wallpapers.Count)
        {
            return;
        }
        for (int i = 0; i < wallpaperHandler.wallpapers.Count; i++)
        {
            ...
            wallpaperItemController.SetData(wallpaperHandler.wallpapers[i]);
        }
    }
    ...
    public void GenerateShirtCell()
    {
        for (int i = 0; i < m_maxShirts; i++)
        {
            ...
            CellController cellController =g.GetComponent<CellController>();
            ...
            cellController.m_StyleImg.sprite = sprite;
            var index = i + 1;
            if (cellController.m_StyleText)
            {
                cellController.m_StyleText.text = $"Shirt "  + (index <= 9 ? "0" + index : index.ToString());
            }
            ShirtsCell.Add(g);
        }

      //  m_ShirtParent.GetComponent<RectTransform>().localPosition = Vector3.zero;
    }
    public void GeneratePantCell()
    {
        for (int i = 0; i < m_maxPants; i++)
        {
            ...
            if (cellController.m_StyleText)
            {
                cellController.m_StyleText.text = $"Pant "  + (index <= 9 ? "0" + index : index.ToString());
            }
            PantsCell.Add(g);
        }
    //    m_PantParent.GetComponent<RectTransform>().localPosition= Vector3.zero;
    }
    ...
    public void GenerateTopSkinsCell()
    {
        for (int i = 1; i < m_maxSkins; i++)
        {
            GameObject g = Instantiate(m_skinPrefab, m_SkinParent);
            SkinController skinController =g.GetComponent<SkinController>();
            var sprite = Resources.Load<Sprite>($"textures/body_shirt/{i}" );
            int x = i;
           // g.GetComponent<Button>().onClick.AddListener(delegate { SetModel(x); });
           g.GetComponent<Button>().onClick.AddListener(delegate { setModelFullbody(0,x-1); });
           g.GetComponent<Button>().onClick.AddListener(delegate { highLightFullbodycell(x-1);});
           g.GetComponent<Button>().onClick.AddListener(delegate {  ShowFullbodyCatalog();});
            skinController.m_StyleImg.sprite = sprite;
            skinController.m_Styletext.text = "Shirt";
            FullbodyCell.Add(g);
        }
        for (int i = 1; i < m_maxSkins; i++)
        {
            ...
            var sprite = Resources.Load<Sprite>($"textures/body_pant/{i}");
            int x = i;
            int cellindex =m_maxSkins + x;
          //  g.GetComponent<Button>().onClick.AddListener(delegate { SetModel(x); });
          g.GetComponent<Button>().onClick.AddListener(delegate { setModelFullbody(1,x-1); });
          g.GetComponent<Button>().onClick.AddListener(delegate { highLightFullbodycell(cellindex-2);});
          g.GetComponent<Button>().onClick.AddListener(delegate {  ShowFullbodyCatalog();});

            skinController.m_StyleImg.sprite = sprite;
            skinController.m_Styletext.text = "Full Body";

            FullbodyCell.Add(g);
        }
        highLightFullbodycell(0);
    }
    ...
    public void SetModel(int index)
    {
        ...
        if (CurrentCatalog == 0)
        {
            m_bodyController.SetShirtMask(CurrentShirtTexture,CurrentBShirtTexture, CurrentPartTexture,CurrentMaskTexture);
        }
        else if (CurrentCatalog == 2)
        {
            m_bodyController.SettShirtMask(CurrentShirtTexture,CurrentBShirtTexture, CurrentPartTexture,CurrentMaskTexture);
        }
        else
        {
             m_bodyController.SetPantMask(CurrentShirtTexture,CurrentBShirtTexture, CurrentPartTexture,CurrentMaskTexture);
        }
    }

    public void setModelFullbody(int i,int index)
    {
        // Track fullbody skin selection
        ...
        AnalyticsManager.Instance.TrackFeatureUsed("fullbody_selection", fullbodyParams);
...
//        CurrentBShirtTexture = allCategoryTextures[CurrentCatalog].BackMask[CurrentPattern];
        ...
        // Get texture name for analytics if possible
        ...
        m_bodyController.SetFullBodyTexture(FullBodyTexController.textureGroups[i].RawTexture[index]);
        //
        FullBodyTexController.CurrentRawTexture = FullBodyTexController.textureGroups[i].RawTexture[index];
        MenuPanelController.Active3d();
        // m_bodyController.SetFullBodyMask();
    }
