...
public class RobloxBodyHandler : MonoBehaviour
{
    ...
    public static RobloxBodyHandler Instance;
    public void Awake()
    {
        Instance = this;
    }
    ...
    private IEnumerator ChangeCharRotation()
    {
        GameObject g;
    ...
    private void SelectMenu(GameObject g)
    {
    ...
    public void SetShirtMask(Sprite shirt,Sprite bshirt, Sprite part, Texture mask)
    {
        ...
        foreach (var t in m_LegRawImage)
        {
            t.SetActive(false);
        }
        foreach (var t in m_HandRawImage)
        {
            t.SetActive(true);
        }
        foreach (var t in m_LLegBtn)
        {
            t.SetActive(false);
        }
        foreach (var t in m_RLegBtn)
        {
            t.SetActive(false);
        }
        foreach (var t in m_LLegArea)
        {
            t.SetActive(true);
            t.transform.GetChild(0).gameObject.SetActive(false);
        }
        foreach (var t in m_RLegArea)
        {
            t.SetActive(true);
            t.transform.GetChild(0).gameObject.SetActive(false);
        }
    ...
    private void ChangeTexture( Material[] m_Mat,string TextureType, Texture m_Tex)
    {
    ...
    private void ChangeColor( Material[] m_Mat,string TextureType, Color m_Color)
    {
        for (int i = 0; i < m_Mat.Length; i++)
        {
    ...
    public void ClearTexture()
    {
        ...
        {
            SubTabController.Instance.m_colorPlate.gameObject.SetActive(false);
        }
